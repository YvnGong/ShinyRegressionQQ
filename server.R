  ...
  },
  {
    ...
    output$plotleft1 <- renderPlot({
      curve(dgamma(-x, shape = input$leftskew, beta = 1),
            main=cbind(input$pp_plot_title), col="orange", xlab="value", ylab="density",lwd = 5,
    ...
    output$plotleft2 <- renderPlot({
      ...
      if(input$leftpath == 1){
        ...
        qqPlot((matrix[,1] - mean(matrix[,1]))/sd(matrix[,1]), 
               ...
               main = cbind(input$plot_title), ylab = "Standardized Sample Quantiles")
        
        par(new = TRUE)
        qqPlot((matrix[,2] - mean(matrix[,2]))/sd(matrix[,2]), 
               distribution="norm", param.list=list(mean=0, sd=1), 
               points.col = rgb(255, 102, 102, maxColorValue = 255), 
               plot.type = "Q-Q", yaxt='n', xaxt='n',
               add.line=FALSE, cex.lab=1.5, cex.axis=1.5, 
               cex.main=1.5, cex.sub=1.5, qq.line.type="0-1",
               main = cbind(input$plot_title), ylab = "Standardized Sample Quantiles")
      }
        ...
        qqPlot((matrix[,1] - mean(matrix[,1]))/sd(matrix[,1]), 
               distribution="norm", param.list=list(mean=0, sd=1), 
               points.col = rgb(99, 235, 235, maxColorValue = 255), 
               line.col = "orange", plot.type = "Q-Q", qq.line.type="0-1", 
               add.line=TRUE, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5,
               main = cbind(input$plot_title), ylab = "Standardized Sample Quantiles")
        ...
        qqPlot((matrix[,3] - mean(matrix[,3]))/sd(matrix[,3]), 
               distribution="norm", param.list=list(mean=0, sd=1), 
               points.col = rgb(138, 43, 226, maxColorValue = 255), 
               plot.type = "Q-Q", yaxt='n', xaxt='n', qq.line.type="0-1",
    ...
    output$plotright1<-renderPlot({
      curve(dgamma(x, shape = input$rightskew, beta = 1),
            main = cbind(input$pp_plot_title), col="orange", 
            xlab="value", ylab="density",lwd = 5,
            cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5,
    ...
    output$plotright2 <- 
      ...
      if(input$rightpath == 1){
        ...
        qqPlot((matrix[,2] - mean(matrix[,2]))/sd(matrix[,2]), 
               ...
               points.col = rgb(255, 102, 102, maxColorValue = 255), 
               plot.type = "Q-Q", yaxt='n', xaxt='n',
               add.line=FALSE, cex.lab=1.5, cex.axis=1.5, cex.main=1.5, 
               cex.sub=1.5, qq.line.type="0-1",
               main = cbind(input$plot_title), ylab = "Standardized Sample Quantiles")
        ...
        qqPlot((matrix[,1] - mean(matrix[,1]))/sd(matrix[,1]), 
               ...
               points.col = rgb(99, 235, 235, maxColorValue = 255), 
               line.col = "orange", plot.type = "Q-Q", 
               qq.line.type="0-1", add.line=TRUE, cex.lab=1.5, cex.axis=1.5, 
    ...
    output$plotsymmetric1 <- renderPlot({
      ...
      if(input$inverse == 1){
        plot(x, dens, type = "l", yaxs = "i", xaxs = "i", xlim=c(-0.03,1.03),
             cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5,
             xlab = "value", ylab = "density", main = cbind(input$pp_plot_title),
             col = "orange", lwd = 5)
    ...
    data4 <- reactive(matrix(mix.synthetic.facing.gamma(N = input$bisize*input$bipath, 
    ...
    data5 <- 
      reactive(matrix(rnorm(n = input$normpath*input$normsize, 
